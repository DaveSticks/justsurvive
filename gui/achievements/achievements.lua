...
function panel(state)
	if not GUIEdit.staticimage[1] then
		...
		GUIEdit.label[1] = guiCreateLabel(0.34, 0.16, 0.62, 0.17, "Logro Desbloqueado", true, GUIEdit.staticimage[1])
				...
				if(counter == #all["conditions"]) then
					...
					for index, element in ipairs(all["items"]) do
						if getElementData(localPlayer,"CURRENT_Slots") + getItemSlots(element[1]) > getElementData(localPlayer,"MAX_Slots") then
							startRollMessage2("Inventory","Inventario lleno, no puedes recibir las recompensas.",255,0,0)
							break
						else
...
function achievpanel(state)
	if not GUIAchiev.staticimage[1] then
		...
		GUIAchiev.label[1] = guiCreateLabel(0.09, 0.09, 0.14, 0.05, "Logros", true, GUIAchiev.staticimage[1])
		guiSetFont(GUIAchiev.label[1], font0_needhelp)
		guiLabelSetColor(GUIAchiev.label[1], 0, 0, 0)
		
		GUIAchiev.label[2] = guiCreateLabel(0.23, 0.11, 0.21, 0.03, "Tus Logros: 01 de 100 (01% completado)", true, GUIAchiev.staticimage[1])
		guiLabelSetColor(GUIAchiev.label[2], 45, 45, 45)
		
		GUIAchiev.label[3] = guiCreateLabel(0.80, 0.09, 0.14, 0.05, "Cerrar", true, GUIAchiev.staticimage[1])
...
function loadList()  -- Needs optimizing (urgent)
	...
	guiSetText(GUIAchiev.label[2], "Tus Logros: "..aunlocked.." de "..aachiev.." ("..math.round((((aunlocked/aachiev) * 100) or 0),2).."% completado)")
