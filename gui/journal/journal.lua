...
function showJournal()
	if getElementData(localPlayer,"logedin") then
		if not isJournalOpen then
			...
			guiSetText(JournalTable.label[1],"Dia "..getElementData(localPlayer,"daysalive"))
...
function initJournal()
	...
	JournalTable.label[1] = guiCreateLabel(0.09, 0.09, 0.40, 0.06, "Dia 0", true, JournalTable.image[1])
	...
	JournalTable.label[5] = guiCreateLabel(0.51, 0.89, 0.21, 0.05, "Escribir", true, JournalTable.image[1])
	...
	JournalTable.label[7] = guiCreateLabel(0.75, 0.89, 0.21, 0.05, "Siguiente Pagina", true, JournalTable.image[1])
	...
	JournalTable.label[8] = guiCreateLabel(0.51, 0.89, 0.21, 0.05, "Pagina Anterior", true, JournalTable.image[2])
	...
	JournalTable.label[9] = guiCreateLabel(0.75, 0.89, 0.21, 0.05, "Logros", true, JournalTable.image[2])
	...
	JournalTable.label[10] = guiCreateLabel(0.13, 0.15, 0.33, 0.14, "Estadisticas", true, JournalTable.image[2])
	...
	JournalTable.label[11] = guiCreateLabel(0.10, 0.42, 0.14, 0.03, "Sangre/Tipo:", true, JournalTable.image[2])
	guiLabelSetColor(JournalTable.label[11],0,0,0)
	guiSetFont(JournalTable.label[11],font[3])
	
	JournalTable.label[12] = guiCreateLabel(0.10, 0.46, 0.14, 0.03, "Hambre:", true, JournalTable.image[2])
	guiLabelSetColor(JournalTable.label[12],0,0,0)
	guiSetFont(JournalTable.label[12],font[3])
	
	JournalTable.label[13] = guiCreateLabel(0.10, 0.50, 0.14, 0.03, "Sed:", true, JournalTable.image[2])
	guiLabelSetColor(JournalTable.label[13],0,0,0)
	guiSetFont(JournalTable.label[13],font[3])
	
	JournalTable.label[14] = guiCreateLabel(0.10, 0.54, 0.14, 0.03, "Temperatura:", true, JournalTable.image[2])
	guiLabelSetColor(JournalTable.label[14],0,0,0)
	guiSetFont(JournalTable.label[14],font[3])
	
	JournalTable.label[15] = guiCreateLabel(0.10, 0.58, 0.14, 0.03, "Humanidad:", true, JournalTable.image[2])
	guiLabelSetColor(JournalTable.label[15],0,0,0)
	guiSetFont(JournalTable.label[15],font[3])
	
	JournalTable.label[16] = guiCreateLabel(0.10, 0.62, 0.14, 0.03, "Infectados:", true, JournalTable.image[2])
	guiLabelSetColor(JournalTable.label[16],0,0,0)
	guiSetFont(JournalTable.label[16],font[3])
	
	JournalTable.label[17] = guiCreateLabel(0.10, 0.77, 0.36, 0.03, "Dias vivo: 0 Dias", true, JournalTable.image[2])
	...
	JournalTable.label[19] = guiCreateLabel(0.13, 0.31, 0.33, 0.03, "Dificultad:", true, JournalTable.image[2])
	...
	JournalTable.window[1] = guiCreateWindow(0.24, 0.14, 0.52, 0.80, "Escribir", true)
	...
	JournalTable.label[6] = guiCreateLabel(0.04, 0.10, 0.91, 0.08, "Escribe Aqui:", true, JournalTable.window[1])
	guiLabelSetVerticalAlign(JournalTable.label[1], "center")
	JournalTable.button[1] = guiCreateButton(0.17, 0.92, 0.19, 0.06, "Guardar", true, JournalTable.window[1])
	JournalTable.button[2] = guiCreateButton(0.63, 0.92, 0.19, 0.06, "Cancelar", true, JournalTable.window[1])    
...
function nextPreviousPage(number)
	if number then
		...
		if number == 7 then
			...
			guiSetText(JournalTable.label[18],"FPS: "..math.floor(getCurrentFPS()))
			guiSetText(JournalTable.label[19],"Dificultad: "..tostring(gameplayVariables["difficulty"]))
			guiSetText(JournalTable.label[17],"Dias Vivo: "..tostring(getElementData(localPlayer,"daysalive")).." Dias")
	...
	if attacker then
		if getElementType(attacker) == "ped" then
			...
			if spacercount == 16 then
				guiSetText(JournalTable.label[3],"Un infectado me ha atacado, esperemos que no me haya mordido.")
				spacercount = 0
			else
				guiSetText(JournalTable.label[3],guiGetText(JournalTable.label[3])..""..spacer.."Un infectado me ha atacado, esperemos que no me haya mordido.")
			end
		elseif getElementType(attacker) == "player" then
			spacercount = spacercount+1
			if spacercount == 16 then
				guiSetText(JournalTable.label[3],"¡Alguien me ha disparado!")
				spacercount = 0
			else
				guiSetText(JournalTable.label[3],guiGetText(JournalTable.label[3])..spacer.."¡Alguien me ha disparado!")
			end
		end
	end
end
	...
	if getElementData(localPlayer,"logedin") then
		if getElementData(localPlayer,"logedin") then
			if not loginstatus then
				...
				spacercount = spacercount+1
				if spacercount == 16 then
					guiSetText(JournalTable.label[3],"He despertado a las "..hour..":"..minutes..". Parece que no es un sueño todo esto.")
					loginstatus = true
					spacercount = 0
				else
					guiSetText(JournalTable.label[3],guiGetText(JournalTable.label[3])..spacer.."He despertado a las "..hour..":"..minutes..". Parece que no es un sueño todo esto.")
					loginstatus = true
				end
			end
		end
		if getElementData(localPlayer,"thirst") < 20 then
			if not thirststatus then
				spacercount = spacercount+1
				if spacercount == 16 then
					guiSetText(JournalTable.label[3],"Tengo demasiada sed, me tomaria un oceano.")
					thirststatus = true
					spacercount = 0
				else
					guiSetText(JournalTable.label[3],guiGetText(JournalTable.label[3])..spacer.."Tengo demasiada sed, me tomaria un oceano.")
					thirststatus = true
				end
			end
		else
		...
		if getElementData(localPlayer,"food") < 20 then
			if not foodstatus then
				spacercount = spacercount+1
				if spacercount == 16 then
					guiSetText(JournalTable.label[3],"Morire de hambre a este ritmo, necesito comida.")
					foodstatus = true
					spacercount = 0
				else
					guiSetText(JournalTable.label[3],guiGetText(JournalTable.label[3])..spacer.."Morire de hambre a este ritmo, necesito comida.")
					foodstatus = true
				end
			end
		else
		...
		if getElementData(localPlayer,"blood") < 6000 then
			if not bloodstatus1 then
				spacercount = spacercount+1
				if spacercount == 16 then
					guiSetText(JournalTable.label[3],"Me siento debil.")
					bloodstatus1 = true
					spacercount = 0
				else
					guiSetText(JournalTable.label[3],guiGetText(JournalTable.label[3])..spacer.."Me siento debil.")
					bloodstatus1 = true
				end
			end
		else
		...
		if getElementData(localPlayer,"blood") < 1000 then
			if not bloodstatus2 then
				spacercount = spacercount+1
				if spacercount == 16 then
					guiSetText(JournalTable.label[3],"Ya no aguanto mas, creo que ha llegado mi hora...")
					bloodstatus2 = true
					spacercount = 0
				else
					guiSetText(JournalTable.label[3],guiGetText(JournalTable.label[3])..spacer.."Ya no aguanto mas, creo que ha llegado mi hora...")
					bloodstatus2 = true
				end
			end
		else
		...
		if getElementData(localPlayer,"brokenbone") then
			if not bonestatus then
				spacercount = spacercount+1
				if spacercount == 16 then
					guiSetText(JournalTable.label[3],"Me he roto la pierta, mierda.")
					bonestatus = true
					spacercount = 0
				else
					guiSetText(JournalTable.label[3],guiGetText(JournalTable.label[3])..spacer.."Me he roto la pierta, mierda.")
					bonestatus = true
				end
			end
		else
		...
		if getElementData(localPlayer,"cold") then
			if not coldstatus then
				spacercount = spacercount+1
				if spacercount == 16 then
					guiSetText(JournalTable.label[3],"Tiemblo como loco. Debe ser un resfrio, tengo que tomarmelo con calma.")
					coldstatus = true
					spacercount = 0
				else
					guiSetText(JournalTable.label[3],guiGetText(JournalTable.label[3])..spacer.."Tiemblo como loco. Debe ser un resfrio, tengo que tomarmelo con calma.")
					coldstatus = true
				end
			end
		else
		...
		if getElementData(localPlayer,"murders") == 1 then
			if not murderstatus then
				spacercount = spacercount+1
				if spacercount == 16 then
					guiSetText(JournalTable.label[3],"He matado un humano hoy, ¿que he hecho?... Mierda.")
					murderstatus = true
					spacercount = 0
				else
					guiSetText(JournalTable.label[3],guiGetText(JournalTable.label[3])..spacer.."He matado un humano hoy, ¿que he hecho?... Mierda.")
					murderstatus = true
				end
			end
		else
		...
		if getElementData(localPlayer,"humanity") == 5000 then
			if not humanitystatus then
				spacercount = spacercount+1
				if spacercount == 16 then
					guiSetText(JournalTable.label[3],"¡Me siento como un heroe ayudando tanta gente!")
					humanitystatus = true
					spacercount = 0
				else
					guiSetText(JournalTable.label[3],guiGetText(JournalTable.label[3])..spacer.."¡Me siento como un heroe ayudando tanta gente!")
					humanitystatus = true
				end
			end
		else
