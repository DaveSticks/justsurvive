...
function onPlayerEnterDayzVehicle(veh,seat)
	...
	if (getElementData(col,"fuel") or 0) < 0 then
		if getElementModel(veh) ~= 509 then
			triggerClientEvent (source, "displayClientInfo", source,"Vehicle","Vehiculo sin gasolina",22,255,0)
			setVehicleEngineState ( veh, false )
			return
		end
	end
	...
	if getElementModel(veh) ~= 509 then
		...
		outputChatBox("Presiona K para encender/apagar el motor",source)
	end
end
...
function repairVehicle (veh)
	if repairTimer[veh] then triggerClientEvent (source, "displayClientInfo", source,"Vehicle",name.." esta siendo reparado",255,0,0) return end
	...
	if getElementData(source,"Toolbox") and getElementData(source,"Toolbox") > 0 and getElementData(source,"Scrap Metal") and getElementData(source,"Scrap Metal") > 0 then
		...
		triggerClientEvent (source, "displayClientInfo", source,"Vehicle","Comenzaste a reparar "..name,0,255,0)
	else
		triggerClientEvent (source, "displayClientInfo", source,"Vehicle","Necesitas Scrap Metal para reparar el vehiculo",255,0,0)
	end
end
...
function fixVehicleDayZ(veh,player,name)
	...
	if scrap then
		...
		triggerClientEvent (player, "displayClientInfo", player,"Vehicle","Terminaste de reparar "..name,0,255,0)
	end
end
...
function stopFixxingWhileMoving()
	...
	triggerClientEvent (source, "displayClientInfo", source,"Vehicle","Detuviste la reparacion de "..getVehicleName(veh),255,22,0)
...
function setEngineStateByPlayer (playersource)
	...
	if getVehicleEngineState(veh) then
		triggerClientEvent (playersource, "displayClientInfo", playersource,"Vehicle","Motor encendido",22,255,0)
	else
		triggerClientEvent (playersource, "displayClientInfo", playersource,"Vehicle","Motor apagado",255,22,0)
	end
end
	...
	if getElementData(source,"Toolbox") > 0 then
		...
		setTimer(function(source,veh,action)
			if action == "takewheel" then
				...
				triggerClientEvent (source, "displayClientInfo", source,"Vehicle","Has sacado una rueda",0,255,0)
			elseif action == "takeengine" then
				...
				triggerClientEvent (source, "displayClientInfo", source,"Vehicle","Has sacado el motor",0,255,0)
			elseif action == "siphon" then
				...
				triggerClientEvent (source, "displayClientInfo", source,"Vehicle","Has sifoneado algo de combustible",0,255,0)
			end
		end,3000,1,source,veh,action)
	else
		triggerClientEvent (source, "displayClientInfo", source,"Vehicle","Necesitas una caja de herramientas",255,0,0)
	end
end
